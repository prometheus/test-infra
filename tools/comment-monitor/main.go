...
func handleErr(w http.ResponseWriter, logger *slog.Logger, errMsg string, statusCode int, err error) {
	...
	http.Error(w, "comment-monitor configuration or secrets are incorrect; check logs", statusCode)
}
...
func (d *dispatcher) HandleIssue(w http.ResponseWriter, r *http.Request) {
	...
	ghClient, err := internal.NewGithubClient(r.Context(), ghToken)
	if err != nil {
	...
	if parseErr != nil {
		if comment := parseErr.ToComment(); comment != "" {
			if postErr := ghClient.PostComment(comment); postErr != nil {
				...
				handleErr(w, logger, "could not post comment to GitHub on failed command parsing", http.StatusBadRequest, err)
				return
			}
		}
